...
public final class HGTDigitalElevationModel implements DigitalElevationModel {
    ...
    public HGTDigitalElevationModel(File file) throws IOException, IllegalArgumentException {
        ...
        Preconditions.checkArgument(sideLength > 0, "Invalid file size.");

        longitude = ((fileName.charAt(3) == 'W') ? -1 : 1) * Integer.parseInt(fileName.substring(4, 7));
    ...
    public Vector3 normalAt(PointGeo p) throws IllegalArgumentException {
        FileCoords preciseCoords = fileCoords(p);

        int i = lowerCellIndex(preciseCoords.preciseI());
        int j = lowerCellIndex(preciseCoords.preciseJ());
    ...
    private int lowerCellIndex(double preciseCoordinate) {
        return Math.min((int) preciseCoordinate, sideLength - 1);
    }

    private FileCoords fileCoords(PointGeo p) {
